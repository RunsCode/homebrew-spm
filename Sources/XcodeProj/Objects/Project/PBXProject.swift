...
public final class PBXProject: PBXObject {
    ...
    @discardableResult
    public func removeSwiftPackage(repositoryURL: String,
                                   productName: String,
                                   targetName: String) -> XCRemoteSwiftPackageReference? {
        do {
            let objects = try objects()
    ...
    //            throw PBXProjError.targetNotFound(targetName: targetName)
            ...
            guard let reference = try removeSwiftPackageReference(repositoryURL: repositoryURL) else {
                return nil
    //            throw PBXObjectError.objectNotFound(repositoryURL)
            }
            guard let product = try removeSwiftPackageProduct(reference: reference, productName: productName, target: target) else {
                return nil
    //            throw PBXObjectError.objectNotFound(reference.name ?? productName)
            }

            let buildFile = PBXBuildFile(product: product)
            objects.delete(reference: buildFile.reference)
    ...
    //            throw PBXProjError.frameworksBuildPhaseNotFound(targetName: targetName)
            ...
            frameworksBuildPhase.files?.removeAll {
                $0 == buildFile
            }
            return reference
    ...
    public func removeLocalSwiftPackage(path: Path,
                                        productName: String,
                                        targetName: String) throws -> XCSwiftPackageProductDependency {
        guard path.isRelative else { throw PBXProjError.pathIsAbsolute(path) }
        ...
        guard let productDependency = try removeLocalSwiftPackageProduct(path: path, productName: productName, target: target) else {
            throw PBXObjectError.objectNotFound(productName)
        }
        let buildFile = PBXBuildFile(product: productDependency)
        objects.delete(reference: buildFile.reference)
        //
        ...
        let reference = PBXFileReference(sourceTree: .group,
                                         ...
                                         lastKnownFileType: "folder",
                                         path: path.string)
        objects.delete(reference: reference.reference)
        mainGroup.children.removeAll {
            $0 == reference
        }
        return productDependency
    }
...
extension PBXProject {
    ...
    private func removeSwiftPackageReference(repositoryURL: String) throws -> XCRemoteSwiftPackageReference? {
        guard let package = remotePackages.first(where: { $0.repositoryURL == repositoryURL }) else {
            return nil
        }
        try objects().delete(reference: package.reference)
        remotePackages.removeAll { $0 == package }
        return package
    }
    ...
    private func removeSwiftPackageProduct(reference: XCRemoteSwiftPackageReference,
                                           productName: String,
                                           target: PBXTarget) throws -> XCSwiftPackageProductDependency? {
        let objects = try objects()
        guard let dependency = objects.swiftPackageProductDependencies
            .first(where: { $0.value.package == reference && $0.value.productName == productName })?.value else {
            return nil
        }
        objects.delete(reference: dependency.reference)
    ...
    private func removeLocalSwiftPackageProduct(path: Path,
                                                productName: String,
                                                target: PBXTarget) throws -> XCSwiftPackageProductDependency? {
        let objects = try objects()
        guard let dependency = objects.swiftPackageProductDependencies
            .first(where: { $0.value.productName == productName })?.value else {
            return nil
        }
        objects.delete(reference: dependency.reference)
        target.packageProductDependencies?.removeAll { $0 == dependency}
